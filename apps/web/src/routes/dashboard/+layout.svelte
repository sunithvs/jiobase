<script lang="ts">
	...
	onMount(() => {
		...
		const interval = setInterval(() => {
			const last = localStorage.getItem('jb_last_donation_modal');
			const now = Date.now();
			if (!last || now - Number(last) > INTERVAL_MS) {
				showDonation = true;
				localStorage.setItem('jb_last_donation_modal', String(now));
			}
		}, INTERVAL_MS);
...
{:else if auth.user}
	...
	<div class="min-h-screen bg-[#0a0a0a]">
		...
		<!-- Donation banner -->
		<div class="border-b border-white/5 bg-amber-400/5">
			<div class="mx-auto flex max-w-5xl flex-col gap-2 px-6 py-2.5 sm:flex-row sm:items-center sm:justify-between">
				<p class="text-xs sm:text-sm text-gray-400">
					I built JioBase solo and fund it out of pocket. If it's keeping your app alive, <span class="text-amber-300">a quick coffee helps me keep it free.</span>
				</p>
				<a
					href="https://buymeacoffee.com/sunithvs"
					target="_blank"
					rel="noopener"
					class="flex w-fit items-center gap-1.5 rounded-lg border border-amber-400/20 bg-amber-400/10 px-3 py-1.5 text-xs font-medium text-amber-300 transition hover:bg-amber-400/20 shrink-0"
				>
